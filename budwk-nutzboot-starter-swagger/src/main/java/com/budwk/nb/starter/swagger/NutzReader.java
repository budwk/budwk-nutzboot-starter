...
public class NutzReader implements OpenApiReader {
    ...
    public NutzReader(OpenAPIConfiguration openApiConfiguration) {
        this();
    ...
    public OpenAPI read(Set<Class<?>> classes) {
        ...
        Map<Class<?>, NutzReaderListener> listeners = new HashMap<>();

        for (Class<?> cls : sortedClasses) {
            if (ReaderListener.class.isAssignableFrom(cls) && !listeners.containsKey(cls)) {
                try {
                    listeners.put(cls, (NutzReaderListener) cls.newInstance());
                } catch (Exception e) {
        ...
        for (NutzReaderListener listener : listeners.values()) {
            try {
    ...
    public OpenAPI read(Class<?> cls,
        ...
        // class path
        final At apiPath = ReflectionUtils.getAnnotation(cls, At.class);
        final ApiVersion apiVersion = ReflectionUtils.getAnnotation(cls, ApiVersion.class);
        ...
        for (Method method : methods) {
            ...
            if (operationPath != null) {
                ...
                String httpMethod = NutzReaderUtils.extractOperationMethod(method, OpenAPIExtensions.chain());
                ...
                if (operation != null) {
                    ...
                    if (apiFormParams != null) {
                        if (apiFormParams.apiFormParams().length > 0) {
                            List<String> requiredItem = new ArrayList<>();
                            for (ApiFormParam formParam : apiFormParams.apiFormParams()) {
                                ...
                                if (!format.trim().isEmpty()) {
                                    ...
                                    property.setFormat(NutzReaderUtils.getFormatByType(formParam.type()));
                                }

                                String example = formParam.example();
                                ...
                                String description = formParam.description();
                                if (Strings.isNotBlank(description)) {
                                    property.setDescription(description);
                                }

                                String paramName = formParam.name();
                                if (Strings.isNotBlank(paramName)) {
                        ...
                        if (thatClass != null) {
                            ...
                            for (Field field : fields) {
                                ...
                                if (column != null) {
                                    ...
                                    if (comment != null) {
                                        property.setDescription(comment.value());
                                    }
                                    property.setType(NutzReaderUtils.getParamType(field.getType().getTypeName()));
                                    property.setFormat(NutzReaderUtils.getFormatByType(field.getType().getTypeName()));
                                    schema.addProperties(field.getName(), property);
                                }
                            }
                        }
    ...
    protected String resolveApplicationPath() {
        if (application != null) {
            ...
            try {
                ...
                Method m = application.getClass().getMethod("getApplication", (Class<?>) null);
                while (m != null) {
                    Application retrievedApp = (Application) m.invoke(innerApp, (Object) null);
                    ...
                    m = innerApp.getClass().getMethod("getApplication", (Class<?>) null);
                }
    ...
    protected Optional<List<Parameter>> getParametersListFromAnnotation(io.swagger.v3.oas.annotations.Parameter[]
                                                                                parameters, Consumes classConsumes, Consumes methodConsumes, Operation operation, JsonView jsonViewAnnotation) {
        if (parameters == null) {
    ...
    protected ResolvedParameter getParameters(Type type, List<Annotation> annotations, Operation
            operation, Consumes classConsumes,
    ...
    protected Content processContent(Content content, Schema schema, Consumes methodConsumes, Consumes
            classConsumes) {
        if (content == null) {
            content = new Content();
        }
    ...
    protected Class<?> getSubResourceWithNutzSubresourceLocatorSpecs(Method method) {
        ...
        if (method.getAnnotation(At.class) != null) {
            if (NutzReaderUtils.extractOperationMethod(method, null) == null) {
                return type;
            }
        }
        return null;
